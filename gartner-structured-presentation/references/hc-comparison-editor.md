...
方法来自 Gartner-HC-Editor 的交付编辑器：原生 SVG + 单文件 HTML；复用标签、多选、拖动、动态引导线、撤销／恢复、布局保存／载入与 SVG／PNG 导出。仓库保存去业务化的标准 SVG 与实际编辑器模板，保留双曲线几何、颜色、两份图例和采用符号；不带原报告技术数据、企业判断、人员或文档地址。用户指定 SVG／HTML 时以用户原件为准，原件只读。
...
优先按[标准模板构建与验收](hc-template-contract.md)生成 `hc-comparison.svg` 与可编辑 HTML。必须有一张同坐标系、恰好两条年度轨道的完整总览；不能只检查“不同年份集合有两个”。模板的曲线、颜色、线宽、虚线、图例和符号须逐项继承；年份更新为真实报告年份，技术名保持英文原名。

- 两条曲线各自保留峰值、谷底、平台及阶段边界。能读取原图时按原图归一化，不把两张不同尺寸原图直接叠加。
- 只有阶段信息时，可以绘制阶段示意曲线，但图内必须写“阶段示意；位置不代表精确时间或量化分数”。不从阶段等级编造连续坐标变化证据。
- 同一技术前后年点位用迁移连线关联，方向是前年到后年；来源归一化位置相同则不画迁移线，双轨道本身的视觉间距不是变化证据。无精确点位证据时只表达阶段变化。新增、移出使用模板既定符号与图例。
- 符号形状映射各年自身的主流采用年限，不能把后年符号强加给前年；收益、关注优先级和迁移幅度也不可混用。
- 仅标签自动避让，技术点不加避碰位移，迁移线连接原有点位。密集图可附加分面详图，不能代替完整双曲线总览。不得调高引导线阈值掩盖排版失败；模板默认 232 SVG 单位。
...
标准模板优先使用 `build_hc_template.py`，同时生成 SVG／HTML／映射。下列 `build_hc_editor.py` 仅用于已完成模板与来源验收的 SVG 重新打包；它是 DOM／安全检查器，不等于模板验收器。
...
- 用 `validate_hc_template.py` 独立核验模板、点位、年度采用形状、颜色和迁移关系；文档结构脚本通过不能替代图表验收。
